...
<body class="bg-gray-100 min-h-screen text-gray-800 font-sans p-8">
    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div id="ai-tasks-column" class="md:col-span-2 space-y-6">
                <!-- Aktive Queue -->
                <div class="bg-white p-6 rounded shadow">
                    <h2 class="text-xl font-bold mb-4 border-b pb-2">Active Queue</h2>
                    @forelse($tasks->whereIn('status', ['pending', 'running', 'needs_info']) as $task)
                        <div class="mb-4 p-4 border rounded hover:bg-gray-50 flex justify-between items-start @if($task->status === 'needs_info') border-orange-300 bg-orange-50 @endif">
                            <div>
                                <a href="{{ route('ai.show', $task->id) }}" class="hover:underline">
                                    <h3 class="font-bold text-lg text-blue-700">#{{ $task->id }}: {{ $task->title }}</h3>
                                </a>
                                <div class="text-sm text-gray-600 mb-2">Module: {{ $task->module }} | Status: <span
                                        class="font-mono px-2 rounded
                            ...
                            <form action="{{ route('ai.destroy', $task->id) }}" method="POST">
                                ...
                                <button type="submit" class="text-red-500 hover:text-red-700 font-bold"
                                    onclick="return confirm('WIRKLICH LÖSCHEN?')">X</button>
                            </form>
                        </div>
                    @empty
                ...
                <div class="bg-white p-6 rounded shadow opacity-80">
                    ...
                    @forelse($tasks->whereIn('status', ['completed', 'failed']) as $task)
                        <div class="mb-4 p-4 border rounded hover:bg-gray-50 flex justify-between items-start">
                            <div class="flex-grow">
                                <a href="{{ route('ai.show', $task->id) }}" class="hover:underline">
                                    <h3 class="font-bold text-blue-700">#{{ $task->id }}: {{ $task->title }}</h3>
                                </a>
                                <div class="text-sm mb-2">
                                    Status: <span
                                        class="font-mono px-2 rounded {{ $task->status === 'completed' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">{{ $task->status }}</span>
                                </div>


                                @foreach($task->runs as $run)
                                    <div
                                        class="text-xs bg-gray-100 p-2 mt-2 rounded border-l-4 {{ $run->status === 'success' ? 'border-green-500' : ($run->status === 'failed' ? 'border-red-500' : 'border-gray-400') }}">
                                        <strong>Run {{ $run->run_no }} ({{ $run->job_type }}):</strong> {{ $run->status }}
                                        @if($run->git_commit_hash) | Git: {{ $run->git_commit_hash }} @endif
                                        @if($run->stderr_log)
                                            <div class="mt-1 text-red-600 font-mono overflow-x-auto whitespace-pre">
                                                {{ $run->stderr_log }}</div>
                                        @endif
                                    </div>
                                @endforeach
                            </div>
                            <form action="{{ route('ai.destroy', $task->id) }}" method="POST" class="ml-4">
                                @csrf @method('DELETE')
                                <button type="submit" class="text-red-500 hover:text-red-700 font-bold"
                                    onclick="return confirm('WIRKLICH LÖSCHEN?')">X</button>
                            </form>
    ...
    <!-- Live Auto-Reload Script für die rechte Tasks-Spalte -->
    <script>
        setInterval(() => {
            ...
            .then(html => {
                ...
                const newContent = doc.getElementById('ai-tasks-column');
                if (newContent) {
                    document.getElementById('ai-tasks-column').innerHTML = newContent.innerHTML;
                }
            })
